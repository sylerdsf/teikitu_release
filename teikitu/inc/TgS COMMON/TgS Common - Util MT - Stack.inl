...
TgINLINE STg2_UT_ST__ST_Node_P tgCM_UT_LF__ST__Pop( STg2_UT_LF__ST_PCU NONULL psST )
{
    ...
    do
    {
        ...
        sNew.m_pHead = sOrig.m_pHead->m_pNext_Node;
        sNew.m_uiTicket = sOrig.m_uiTicket + 1;
    }
    while (!atomic_compare_exchange_weak( &psST->m_sTop, &sOrig, sNew ));
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgINLINE TgVOID tgCM_UT_MT__ST_MX__Init( STg2_UT_MT__ST_MX_PCU NONULL psMP_ST, TgCHAR_MB_CPCU mbzName )
{
    TgVERIFY( KTgS_OK == tgPM_MT_MX_Init( &psMP_ST->m_sLock, nullptr ) );
    TgVERIFY( KTgS_OK == tgPM_MT_SM_Init( &psMP_ST->m_sStack_Count, mbzName, 0, TgPLATFORM__SEMAPHORE_MAX_VALUE ) );
