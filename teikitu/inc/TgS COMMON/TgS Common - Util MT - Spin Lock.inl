...
TgINLINE TgRESULT tgCM_UT_LF__SN_CNT__Lock_Or_Increment_Count( STg2_UT_LF__SN_CNT_PCU NONULL psMP_SN_CNT )
{
    ...
    do
    {
        if (0 != sOrig.m_nuiTested || (sOrig.m_uiRequest != sOrig.m_nuiServed))
        {
            sNew.m_nuiServed = sOrig.m_nuiServed;
            sNew.m_uiRequest = sOrig.m_uiRequest;
            sNew.m_nuiTested = sOrig.m_nuiTested + 1;
        }
        else
        {
            sNew.m_nuiServed = sOrig.m_uiRequest;
    ...
    return (0 == sNew.m_nuiTested ? KTgS_OK : KTgE_FAIL);
}
...
TgINLINE TgUINT_F32 tgCM_UT_LF__SN_CNT__Signal_Or_Reset_Count( STg2_UT_LF__SN_CNT_PCU NONULL psMP_SN_CNT )
{
    ...
    do
    {
        sNew.m_nuiServed = sOrig.m_nuiServed + (0 == sOrig.m_nuiTested ? 1 : 0);
        sNew.m_uiRequest = sOrig.m_uiRequest;
        sNew.m_nuiTested = 0;
...
TgINLINE TgRESULT tgCM_UT_LF__SN_ID__Init( STg2_UT_LF__SN_ID_PCU NONULL psMP_SN_ID, TgUINT_E64_C uiKI )
{
    ...
    atomic_store( &psMP_SN_ID->m_uiSRV_REQ_ID, sSRV_REQ_ID.m_uiSRV_REQ_ID );
    return (KTgS_OK);
}
...
TgINLINE TgVOID tgCM_UT_LF__SN_ID__Free_Yield( STg2_UT_LF__SN_ID_PCU NONULL psMP_SN_ID )
{
    ...
    sOrig.m_uiSRV_REQ_ID = atomic_load( &psMP_SN_ID->m_uiSRV_REQ_ID );
    do
    {
        sNew.m_uiSRV_REQ = sOrig.m_uiSRV_REQ;
        sNew.m_uiKI = KTgID__INVALID_VALUE;
    }
    while(!atomic_compare_exchange_weak( &psMP_SN_ID->m_uiSRV_REQ_ID, &sOrig.m_uiSRV_REQ_ID, sNew.m_uiSRV_REQ_ID ));
    ...
    while(1)
    {
        ...
        if (atomic_compare_exchange_weak( &psMP_SN_ID->m_uiSRV_REQ_ID, &sTest.m_uiSRV_REQ_ID, sNew.m_uiSRV_REQ_ID ))
        {
            break;
        };
...
//    sOrig.m_uiSRV_REQ_ID = sNew.m_uiSRV_REQ_ID;
//    sTest.m_uiSRV_REQ_ID = sNew.m_uiSRV_REQ_ID;
//    sNew.m_uiSRV_REQ = 0;
//    while (!atomic_compare_exchange_weak( &psMP_SN_ID->m_uiSRV_REQ_ID, &sTest.m_uiSRV_REQ_ID, sNew.m_uiSRV_REQ_ID ))
//    {
//        sTest.m_uiSRV_REQ_ID = sOrig.m_uiSRV_REQ_ID;
//    }
    //atomic_store( &psMP_SN_ID->m_uiSRV_REQ_ID, sNew.m_uiSRV_REQ_ID );
...
#if TgCOMPILE_ASSERT && TgCOMPILE_ASSERT__VERIFY
    sSRV_REQ_ID.m_uiSRV_REQ_ID = atomic_load( &psMP_SN_ID->m_uiSRV_REQ_ID );
...
TgINLINE TgRESULT tgCM_UT_LF__SN_ID__Lock_Yield( STg2_UT_LF__SN_ID_PCU NONULL psMP_SN_ID, TgUINT_F64_C uiKI )
{
    ...
    sCurrent.m_uiSRV_REQ_ID = sNew.m_uiSRV_REQ_ID;

    while (sCurrent.m_nuiServed != sOrig.m_uiRequest)
    {
        tgTR_Yield();
        sCurrent.m_uiSRV_REQ_ID = atomic_load( &psMP_SN_ID->m_uiSRV_REQ_ID );
...
TgINLINE TgRESULT tgCM_UT_LF__SN_ID__Lock_Test( STg2_UT_LF__SN_ID_PCU NONULL psMP_SN_ID, TgUINT_F64_C uiKI )
{
    ...
    if (atomic_compare_exchange_strong( &psMP_SN_ID->m_uiSRV_REQ_ID, &sTest.m_uiSRV_REQ_ID, sNew.m_uiSRV_REQ_ID ))
    {
        atomic_thread_fence( memory_order_seq_cst );
...
{
    STg2_UT_ST__SRV_REQ_ID              sCurrent;
    TgRESULT                            iResult;
    ...
    sCurrent.m_uiSRV_REQ_ID = atomic_load( &psMP_SN_ID->m_uiSRV_REQ_ID );
    sCurrent.m_uiI = KTgMAX_U32;
    atomic_store( &psMP_SN_ID->m_uiSRV_REQ_ID, sCurrent.m_uiSRV_REQ_ID);
