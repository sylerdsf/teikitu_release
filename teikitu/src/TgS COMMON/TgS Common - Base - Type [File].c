...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Add_Separator( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyDest;

    TgPARAM_CHECK((nullptr != mbzDest) &&(nbyMaxDest > 0));

    nbyDest = tgMBZ_Length_U08( mbzDest, nbyMaxDest );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Add_Extension( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest, TgCHAR_MB_CPC NONULL mbzExt, TgRSIZE_C nbyMaxExt )
{
    TgRSIZE                                 nbyDest;
    TgRSIZE                                 nbyExt;
    ...
    nbyDest = tgMBZ_Length_U08( mbzDest, nbyMaxDest );
    nbyExt = tgMBZ_Length_U08( mbzExt, nbyMaxExt );
    ...
    if (TgFAILED(tgMBZ_Append( mbzDest, nbyMaxDest, mbzExt, nbyMaxExt )))
    {
        return (KTgE_FAIL);
    };
...
/* ---- tgCM_Path_Copy ----------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Copy( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest, TgCHAR_MB_CPC NONULL mbzSrc, TgRSIZE_C nbyMaxSrc )
{
...
/* ---- tgCM_Path_Append --------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Append( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest, TgCHAR_MB_CPC NONULL mbzSrc, TgRSIZE_C nbyMaxSrc )
{
    TgRSIZE                                 nbyDest;
    TgRSIZE                                 nbySrc;
    TgRESULT                                iResult;
    ...
    nbyDest = tgMBZ_Length_U08( mbzDest, nbyMaxDest );
    nbySrc = tgMBZ_Length_U08( mbzSrc, nbyMaxSrc );
    ...
    if (nbyDest > 0)
    {
        ...
        {
            iResult = tgMBZ_Append( mbzDest, nbyMaxDest, mbzSrc + 1, nbyMaxSrc - 1);
        }
        else
        {
            iResult = tgMBZ_Append( mbzDest, nbyMaxDest, mbzSrc, nbyMaxSrc );
        };
    }
    else
    {
        iResult = tgMBZ_Copy( mbzDest, nbyMaxDest, mbzSrc, nbyMaxSrc );
    };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRSIZE tgCM_Path_Common_Prefix_Size_U08( TgCHAR_MB_CP NONULL mbzPath0, TgRSIZE nbyMaxPath0, TgCHAR_MB_CP NONULL mbzPath1, TgRSIZE nbyMaxPath1 )
{
    ...
    {
        TgCHAR_MB_C                         mbCode0 = *mbzPath0;
        TgCHAR_MB_C                         mbCode1 = *mbzPath1;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRSIZE tgCM_Path_Common_Prefix_Size_Count( TgCHAR_MB_CP NONULL mbzPath0, TgRSIZE nbyMaxPath0, TgCHAR_MB_CP NONULL mbzPath1, TgRSIZE nbyMaxPath1 )
{
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Common_Prefix( TgCHAR_MB_P mbzDest, TgRSIZE nbyMaxDest, TgCHAR_MB_CP NONULL mbzP0, TgRSIZE nbyMaxP0, TgCHAR_MB_CP NONULL mbzP1, TgRSIZE nbyMaxP1 )
{
    TgCHAR_MB_P                             mbzNull;
    ...
    {
        TgCHAR_MB_C                         mbCode0 = *mbzP0;
        TgCHAR_MB_C                         mbCode1 = *mbzP1;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Remove_Separator( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyDest;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Remove_Extension( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyOffset;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Remove_File_Name( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyOffset;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Rename_Extension( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE_C nbyMaxDest, TgCHAR_MB_CPC NONULL mbzNewExt, TgRSIZE_C nbyMaxNewExt )
{
    TgRSIZE                                 nbyNewExt;
    TgRSIZE                                 nbyPrefix;
    ...
    nbyNewExt = tgMBZ_Length_U08( mbzNewExt, nbyMaxNewExt );
    ++nbyPrefix;
    ...
    if (TgFAILED(tgMBZ_Append( mbzDest + nbyPrefix, nbyMaxDest - nbyPrefix, mbzNewExt, nbyMaxNewExt )))
    {
        return (KTgE_FAIL);
    };
    return (KTgS_OK);
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Remove_Directory( TgCHAR_MB_P NONULL mbzDest, TgRSIZE nbyMaxDest )
{
    TgRSIZE                                 nbyOffset;
    ...
    for (TgCHAR_MB_P mbzIter = mbzDest + nbyOffset; *mbzIter; ++mbzIter, ++mbzDest, --nbyMaxDest)
    {
        *mbzDest = *mbzIter;
        if (nbyMaxDest <= 1)
        {
...
/* ---- tgCM_Path_Init ----------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Init( TgCHAR_MB_PC NONULL mbzDest, TgRSIZE nbyMaxDest, ... )
{
    va_list                                 vaList;
    TgCHAR_MB_CP                            mbzPath;
    TgCHAR_MB_P                             mbzIter;

    TgPARAM_CHECK((nullptr != mbzDest) && (nbyMaxDest > 0));

    va_start( vaList, nbyMaxDest );
    mbzPath = (TgCHAR_MB_P)va_arg( vaList, TgCHAR_MB_P );
    mbzIter = mbzDest;
    mbzIter[0] = 0;

    for (; 0 != *mbzPath; mbzPath = (TgCHAR_MB_P)va_arg( vaList, TgCHAR_MB_P ))
    {
        TgRSIZE                                 nbyPathAppend;

        nbyPathAppend = tgMBZ_Length_U08( mbzPath, KTgMAX_SIZE_ALL );
        ...
        if (TgFAILED(tgMBZ_Append( mbzIter, nbyMaxDest, mbzPath, nbyPathAppend )))
        {
            va_end( vaList );
            mbzDest[0] = 0;
            return (KTgE_FAIL);
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Find_Extension( TgRSIZE_P NONULL pnbyOffset, TgCHAR_MB_CPC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyDest;
    TgCHAR_MB_CP                            mbzIter;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgCM_Path_Find_File_Name( TgRSIZE_P NONULL pnbyOffset, TgCHAR_MB_CPC NONULL mbzDest, TgRSIZE_C nbyMaxDest )
{
    TgRSIZE                                 nbyDest;
    TgCHAR_MB_CP                            mbzIter;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgCM_Path_IsAbsolute( TgCHAR_MB_CPC mbzPath, TgRSIZE_C nbyMaxPath )
{
