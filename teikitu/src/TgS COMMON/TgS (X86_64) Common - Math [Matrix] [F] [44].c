...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (X86_64) Common - Math [Matrix] [F] [44].c
...
/* ---- tgMH_CAT_F32_04_4_IMPL ------------------------------------------------------------------------------------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgMH_CAT_F32_04_4_IMPL( TgVEC_F32_04_4_PCU ptmRet, TgVEC_F32_04_4_CPCU pmM0, TgVEC_F32_04_4_CPCU pmM1 )
{
    ...
    const __m128                        mi12 = _mm_shuffle_ps( pmM0->m_avCol[3], pmM0->m_avCol[3], 0x00 );
    const __m128                        mi13 = _mm_shuffle_ps( pmM0->m_avCol[3], pmM0->m_avCol[3], 0x55 );
    const __m128                        mi14 = _mm_shuffle_ps( pmM0->m_avCol[3], pmM0->m_avCol[3], 0xAA );
    const __m128                        mi15 = _mm_shuffle_ps( pmM0->m_avCol[3], pmM0->m_avCol[3], 0xFF );
    ...
    const __m128                        mi19 = _mm_mul_ps( mi12, pmM1->m_avCol[0] );
    ...
    const __m128                        mi23 = _mm_mul_ps( mi13, pmM1->m_avCol[1] );
    ...
    const __m128                        mi27 = _mm_mul_ps( mi14, pmM1->m_avCol[2] );
    const __m128                        mi28 = _mm_mul_ps( mi03, pmM1->m_avCol[3] );
    const __m128                        mi29 = _mm_mul_ps( mi07, pmM1->m_avCol[3] );
    const __m128                        mi30 = _mm_mul_ps( mi11, pmM1->m_avCol[3] );
    const __m128                        mi31 = _mm_mul_ps( mi15, pmM1->m_avCol[3] );
    ...
    ptmRet->m_avCol[3] = _mm_add_ps( _mm_add_ps( mi19, mi23 ), _mm_add_ps( mi27, mi31 ) );
}


/* ---- tgMH_INV_DET_F32_04_4_IMPL -------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgMH_INV_DET_F32_04_4_IMPL( TgVEC_F32_04_4_PCU ptmRet, TgFLOAT32 fDet, TgVEC_F32_04_4_CPCU pmM1 )
{
    ...
    const __m128                        miR3 = pmM1->m_avCol[3];
    ...
    ptmRet->m_avCol[0] = _mm_add_ps( mi020, _mm_add_ps( mi030, mi040 ) );
    ptmRet->m_avCol[1] = _mm_add_ps( mi050, _mm_add_ps( mi060, mi070 ) );
    ptmRet->m_avCol[2] = _mm_add_ps( mi080, _mm_add_ps( mi090, mi100 ) );
    ptmRet->m_avCol[3] = _mm_add_ps( mi110, _mm_add_ps( mi120, mi130 ) );
}


/* ---- tgMH_DET_F32_04_4_IMPL ------------------------------------------------------------------------------------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgFLOAT32 tgMH_DET_F32_04_4_IMPL( TgVEC_F32_04_4_CPCU pmM1 )
{
    const __m128                        miR0 = pmM1->m_avCol[0];
    const __m128                        miR1 = pmM1->m_avCol[1];
    const __m128                        miR2 = pmM1->m_avCol[2];
    ...
    const __m128                        mi52 = _mm_shuffle_ps( mi51, mi51, 0x00 );

    return (*(TgFLOAT32_CP)&mi52);
}
