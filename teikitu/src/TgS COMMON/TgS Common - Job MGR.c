...
TgTYPE_STRUCT(STg2_Job_Queue,)
{
    ...
    TgJOB_QUEUE_ID_SINGLETON                    m_tiQueue_Singleton; /**< Managing and Atomic identifier */
    TgSINT_F64_A                                m_niQueued;
#if 0 != (65584 % TgCCL)
    TgUINT_F08                                  m_uiPad0[TgCCL - (65584 % TgCCL)];
#endif
};
...
static TgUINT_F32                           tgJM_Run_Job_Scheduler( TgUINT_PTR_C );
...
{
    TgSINT_F32                          iIndex;
    TgJOB_QUEUE_ID_SINGLETON            tiTest_Singleton;
    ...
    tgJOB_QUEUE_ID_Init( &tiTest_Singleton, 0 ); /* Warm the ID system specifically to invalidate 0, 0 */

    tgCM_UT_LF__SN__Init( &s_asLock_Job_Queue.m_sLock );
    tgMM_Set_U08_0x00( s_asJob_Queue, sizeof( STg2_Job_Queue ) * KTgMAX_NUM_QUEUE );
    ...
    {
        tgJOB_QUEUE_ID_Invalidate( &s_asJob_Queue[iIndex].m_tiQueue_Singleton );
    };

#if TgCOMPILE__THREAD
    tgCM_UT_LF__SN__Init( &s_asLock_Job_Thread.m_sLock );
    tgMM_Set_U08_0x00( s_asJob_Thread, sizeof( STg2_Job_Thread ) * KTgMAX_NUM_THREAD );
    ...
    {
        s_asJob_Thread[iIndex].m_tiThread_NoSingleton.m_uiKI = KTgID__INVALID_VALUE;
    };
/*# TgCOMPILE__THREAD */
#endif

    g_tiCVAR_Job_MGR_Enabled = tgCN_Var_Init_Bool( "Job_MGR_Enabled", KTgMAX_SIZE_ALL, "When disabled, all jobs are executed immediately (no queuing)", KTgMAX_SIZE_ALL, 0, true );
...
TgRESULT tgJM_Boot_MGR( TgVOID )
{
    ...
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Failed to init OS Job Queue.\n", "Job MGR", "tgJM_Boot_MGR" );
        s_enJob_MGR_State = ETgMODULE_STATE__INITIALIZED;
        return (KTgE_FAIL);
    }
...
TgRESULT tgJM_Stop_MGR( TgVOID )
{
    ...
    for (iIndex = 0; iIndex < KTgMAX_NUM_QUEUE; ++iIndex)
    {
        ...
        if (!tgJOB_QUEUE_ID_Fetch_And_Is_Valid( &tiQueue_NoSingleton, &s_asJob_Queue[iIndex].m_tiQueue_Singleton ))
            continue;
...
TgJOB_QUEUE_ID tgJM_Init_Job_Queue( TgVOID )
{
    ...
    {
        if (tgJOB_QUEUE_ID_Is_Equal_U64( &s_asJob_Queue[iIndex_Queue].m_tiQueue_Singleton, KTgID__INVALID_VALUE ))
            break;
    };

    if (KTgMAX_NUM_QUEUE == iIndex_Queue)
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Exceeded number of allowable job queues.\n", "Job MGR", "tgJM_Init_Job_Queue" );
        tgCM_UT_LF__SN__Signal( &s_asLock_Job_Queue.m_sLock );
        return (KTgJOB_QUEUE_ID__INVALID);
    };

    tiQueue = tgJOB_QUEUE_ID_Init( &s_asJob_Queue[iIndex_Queue].m_tiQueue_Singleton, (TgUINT_F32)iIndex_Queue );
...
TgVOID tgJM_Free_Job_Queue( TgJOB_QUEUE_ID tiQueue )
{
    ...
    tgJOB_QUEUE_ID_Invalidate( &s_asJob_Queue[tiQueue.m_uiI].m_tiQueue_Singleton );
...
TgRESULT tgJM_Queue_Job( TgJOB_QUEUE_ID tiQueue, STg2_Job_P psJobSrc )
{
    ...
    if((tiQueue.m_uiI >= KTgMAX_NUM_QUEUE) || TgFAILED( tgCM_UT_LF__SN_ID__Lock_Spin( &s_asJob_Queue[tiQueue.m_uiI].m_sLock_Queue_Add.m_sLock, tiQueue.m_uiKI ) ))
    {
        ...
        TgERROR_MSGF( 0, "%-16.16s(%-32.32s): Invalid Queue.\n", "Job MGR", "tgJM_Queue_Job" );
        return (KTgE_FAIL);
    };

    /* Could execute the job in place, but decided to leave that to the caller */
    ...
    atomic_fetch_add( &s_asJob_Queue[tiQueue.m_uiI].m_niQueued, 1 );
#endif
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgJOB_THREAD_ID tgJM_Spawn_Job_Thread( TgJOB_QUEUE_ID_P atiQueueList, TgUINT_F32 niQueue, ETgTHREAD_PRIORITY_C enPriority, TgCHAR_MB_CPC szName )
{
#if TgCOMPILE__THREAD
    TgRSIZE                             uiIndex;
    TgJOB_THREAD_ID_SINGLETON           tiJob_Thread_Singleton;
    ...
    {
        TgERROR_MSGF( 0, "%-16.16s(%-32.32s): Exceeded number of allowable job threads.\n", "Job MGR", "tgJM_Spawn_Job_Thread" );
        tgCM_UT_LF__SN__Signal( &s_asLock_Job_Thread.m_sLock );
        return (KTgJOB_THREAD_ID__INVALID);
    };
    ...
    s_asJob_Thread[uiIndex].m_tiThread_NoSingleton = tgJOB_THREAD_ID_Init( &tiJob_Thread_Singleton, (TgUINT_F32)uiIndex );
...
TgVOID tgJM_Stop_Job_Thread( TgJOB_THREAD_ID tiJob_Thread )
{
    ...
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Job Thread ID Mismatch.\n", "Job MGR", "tgJM_Run_Job_Scheduler" );
        tgCM_UT_LF__SN__Signal( &s_asLock_Job_Thread.m_sLock );
        return;
    };
...
#if defined(DEBUG_STg2_Job_Queue)
    atomic_fetch_sub( &s_asJob_Queue[tiQueue.m_uiI].m_niQueued, 1 );
#endif
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
#if TgCOMPILE__THREAD
static TgUINT_F32 tgJM_Run_Job_Scheduler( TgUINT_PTR_C uiJob_Thread )
{
    ...
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Invalid Job Thread ID as Parameter.\n", "Job MGR", "tgJM_Run_Job_Scheduler" );
