...
TgTYPE_UNION(TgMEM_PAGE_HEADER,) /* 32 Byte Header Block */
{
    ...
    struct
    {
        ...
        TgUINT_E32                                  m_uiBlock_Size;             /*  4 Bytes */      /* Modified only on initialization */
        TgUINT_E08                                  m_auiData[];
    };
};
...
static TgRESULT                             tgMM_OS__Size( TgUINT_F32_P, TgUINT_F32_P, TgVOID_CPC );
static TgRESULT                             tgMM_OS__Malloc( TgMEM_INFO_HEADER_P, TgVOID_PP, TgRSIZE_C );
static TgRESULT                             tgMM_OS__Reserve( TgMEM_INFO_HEADER_P, TgVOID_PP, TgRSIZE_C );
static TgRESULT                             tgMM_OS__Commit( TgMEM_INFO_HEADER_P, TgVOID_PP, TgRSIZE_C );
static TgRESULT                             tgMM_OS__Free( TgMEM_INFO_HEADER_P, TgVOID_PC );
...
static TgRESULT                             tgMM_Pool__Size( TgUINT_F32_P, TgUINT_F32_P, TgVOID_CPC );
static TgRESULT                             tgMM_Pool__Malloc( TgMEM_INFO_HEADER_P, TgVOID_PP, TgRSIZE_C );
static TgRESULT                             tgMM_Pool__Free( TgMEM_INFO_HEADER_P, TgVOID_PC );
static TgRESULT                             tgMM_Pool__Realloc( TgMEM_INFO_HEADER_P, TgVOID_PP, TgMEM_INFO_HEADER_P, TgVOID_PC, TgRSIZE_C );
#if TgS_STAT__COMMON
static TgVOID                               tgMM_Pool__Stats( STg2_Output_PC );
static TgVOID                               tgMM_Pool__Stats_Pool( TgRSIZE_C, TgRSIZE_P, TgRSIZE_P, TgRSIZE_P );
/*# TgS_STAT__COMMON */
#endif
static TgBOOL                               tgMM_Pool__Check_Page_Is_Empty( TgMEM_PAGE_HEADER_P );
...
#if defined(TgS_COMPILE__MEMORY_TRACE) && TgS_COMPILE__MEMORY_TRACE
static TgVOID                               tgMM_Print_Trace_Entry( STg2_Output_PC, TgUINT_E08_CPC );
static TgUINT_PTR                           tgMM_Hash_Trace_Entry( TgUINT_PTR_C );
...
#if TgS_STAT__COMMON
static TgCHAR_MB_C                          s_ambzStat_Name[ETgMM_ALLOCATOR_COUNT][8] = { "OS", "Pool", "User 0", "User 1", "User 2", "User 3" };
...
/*# TgS_STAT__COMMON */
    ...
    if (tgGB_CMD_Query_Argument_Index( "-//stat/boot/memory" ) >= 0)
    {
...
{
    TgERROR_MSG( false, "This function should never be executed." );
    return (KTgS_OK);
}
...
#if TgCOMPILE__CONSOLE
    if (tgGB_CMD_Query_Argument_Index( "-//stat/free/memory" ) >= 0)
    {
        tgMM_Stats( &g_sOutCon );
    };
/*# TgCOMPILE__CONSOLE */
...
{
    TgCHAR_MB                           mbzCol[6][32];

    TgERROR(nullptr != psOUT);

    tgIO_PrintF( psOUT, "%8.8s %16.16s %16.16s %16.16s %16.16s %16.16s %16.16s\n",
                 "", "  Allocations  ", "      Max      ", "   Allocated   ", "      Max      ", "    Reserve    ", "      Max      " );
    tgIO_PrintF( psOUT, "%8.8s %16.16s %16.16s %16.16s %16.16s %16.16s %16.16s\n",
                 "", "---------------", "---------------", "---------------", "---------------", "---------------", "---------------" );

    for (TgRSIZE uiIndex = 0; uiIndex < ETgMM_ALLOCATOR_COUNT; ++uiIndex)
    {
        tgMBZ_Formatted_From_U64( mbzCol[0], sizeof( mbzCol[0] ), atomic_load(s_anuiStat_Allocations + uiIndex) );
        tgMBZ_Formatted_From_U64( mbzCol[1], sizeof( mbzCol[1] ), atomic_load(s_anuiStat_Max_Allocations + uiIndex) );
        tgMBZ_Formatted_From_U64( mbzCol[2], sizeof( mbzCol[2] ), atomic_load(s_auiStat_Allocated + uiIndex) );
        tgMBZ_Formatted_From_U64( mbzCol[3], sizeof( mbzCol[3] ), atomic_load(s_auiStat_Max_Allocated + uiIndex) );
        tgMBZ_Formatted_From_U64( mbzCol[4], sizeof( mbzCol[4] ), atomic_load(s_auiStat_Reserved + uiIndex) );
        tgMBZ_Formatted_From_U64( mbzCol[5], sizeof( mbzCol[5] ), atomic_load(s_auiStat_Max_Reserved + uiIndex) );
...
/* ---- tgMM_Size ---------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSIZE_ALL tgMM_Size( ETgMM_ALLOCATOR_C enAllocator, TgVOID_CPC NONULL pMem )
{
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Malloc_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgRSIZE_C uiSize, TgCHAR_MB_CPC mbzF, TgUINT_E32_C uiL )
{
    TgVOID_P                            pReturn;
...
/*# defined(TgS_COMPILE__MEMORY_TRACE) && TgS_COMPILE__MEMORY_TRACE */
    ...
    TgLOG_MEM( "Mem_MGR", "Address: % 16p Size: % 16X File: % 128.128s Line % 5d\n", pReturn, uiSize, mbzF, uiL );
    return (pReturn);
}
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Reserve_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgRSIZE_C uiSize, TgCHAR_MB_CPC mbzF, TgUINT_F32_C uiL )
{
    TgVOID_P                            pReturn;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Commit_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC pMem, TgRSIZE_C uiSize, TgCHAR_MB_CPC mbzF, TgUINT_F32_C uiL )
{
    TgVOID_P                            pReturn;
...
/* ---- tgMM_Commit -------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Commit( ETgMM_ALLOCATOR_C enAllocator, TgVOID_P OKNULL pMem, TgRSIZE_C uiSize )
{
    TgMEM_INFO_HEADER                   sInfo;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgMM_Free_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC pMem, TgCHAR_MB_CPC mbzF, TgUINT_F32_C uiL )
{
...
/*# defined(TgS_COMPILE__MEMORY_TRACE) && TgS_COMPILE__MEMORY_TRACE */
    ...
    TgLOG_MEM( "Mem_MGR", "Address: % 16p                        File: % 128.128s Line % 5d\n", pMem, mbzF, uiL );
}


/* ---- tgMM_Free ---------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgMM_Free( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC NONULL pMem )
{
    TgMEM_INFO_HEADER                   sInfo;
    TgRESULT                            iResult;
    ...
    if (TgSUCCEEDED(iResult))
    {
        ...
        iValue = sInfo.m_bReserved ? sInfo.m_uiSize : 0;
        atomic_fetch_add( s_auiStat_Reserved + enAllocator, -iValue );
        iValue = sInfo.m_bReserved ? 0 : sInfo.m_uiSize;
        atomic_fetch_add( s_auiStat_Allocated + enAllocator, -iValue );
    };
#endif
}
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Realloc_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC pMem, TgRSIZE_C uiSize, TgCHAR_MB_CPC mbzF, TgUINT_F32_C uiL )
{
    TgVOID_P                            pReturn;
...
/* ---- tgMM_Realloc ------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Realloc( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC NONULL pMem, TgRSIZE_C uiSize )
{
    ...
    if (pReturn != pMem)
    {
        ...
        iValue = sOld_Info.m_bReserved ? sOld_Info.m_uiSize : 0;
        atomic_fetch_add( s_auiStat_Reserved + enAllocator, -iValue );
        iValue = sOld_Info.m_bReserved ? 0 : sOld_Info.m_uiSize;
        atomic_fetch_add( s_auiStat_Allocated + enAllocator, -iValue );
    }
#endif
...
static TgVOID tgMM_Print_Trace_Entry( STg2_Output_PC psOUT, TgUINT_E08_CPC puiData )
{
    ...
    TgCHAR_MB                           mbzCol[2][16];

    TgERROR(nullptr != psOUT);
    uMem.puiE08 = puiData;

    tgIO_PrintF( psOUT, "%96.96s %16.16s %16.16s\n", "             File             ", "      Line     ", "      Size     " );
    tgIO_PrintF( psOUT, "%96.96s %16.16s %16.16s\n", "------------------------------", "---------------", "---------------" );

    tgMBZ_Formatted_From_U64( mbzCol[0], sizeof( mbzCol[0] ), (TgUINT_E64)uMem.psEntry->m_uiLine );
    tgMBZ_Formatted_From_U64( mbzCol[1], sizeof( mbzCol[1] ), (TgUINT_E64)uMem.psEntry->m_uiSize );
...
/* ---- tgMM_OS__Size ------------------------------------------------------------------------------------------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_OS__Size( TgUINT_F32_P NONULL pnbyReserve, TgUINT_F32_P NONULL pnbyAllocated, TgVOID_CPC pMem )
{
    union
    {
        TgVOID_CP                           pVoid;
    ...
    uMem.pVoid = pMem;
    if (uMem.uiPTR >= sizeof(TgMEM_OS_HEADER))
    {
        uMem.uiPTR -= sizeof(TgMEM_OS_HEADER);
    };
...
static TgRESULT tgMM_OS__Malloc( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP ppReturn, TgRSIZE_C uiSize )
{
    ...
    if (nullptr == psMem_OS)
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Unable to commit (%d).\n", "Mem_MGR", __FUNCTION__, uiSize );
        return (KTgE_FAIL);
    };

#if TgCOMPILE__MEM_MALLOC_CLEAR
    tgMM_Set_U08( psMem_OS, sizeof(TgMEM_OS_HEADER) + uiSize, (int)TgCOMPILE_MEM_MALLOC_PATTERN );
#endif
...
static TgRESULT tgMM_OS__Reserve( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP ppReturn, TgRSIZE_C uiSize )
{
    ...
    /* Create an OS Allocation for this request */
    psMem_OS = (TgMEM_OS_HEADER_P)tgMM_PM_Virtual_Reserve( sizeof(TgMEM_OS_HEADER) + uiSize );
    if (nullptr == psMem_OS)
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Unable to allocate (%d).\n", "Mem_MGR", __FUNCTION__, uiSize );
        return (KTgE_FAIL);
    };
...
static TgRESULT tgMM_OS__Commit( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP pMem, TgRSIZE_C uiSize )
{
    ...
    *pMem = tgMM_PM_Virtual_Commit( *pMem, uiSize );

    return (nullptr == *pMem ? KTgE_FAIL : KTgS_OK);
}
...
static TgRESULT tgMM_OS__Free( TgMEM_INFO_HEADER_P psInfo, TgVOID_PC pMem )
{
    ...
    tgMM_Set_U08( uMem.pVoid, sizeof(TgMEM_OS_HEADER) + (uMem.psMem_OS->m_sInfo_Header.m_bReserved ? 0 : uMem.psMem_OS->m_sInfo_Header.m_uiSize), (int)TgCOMPILE_MEM_FREE_PATTERN );
#endif
...
/* ---- tgMM_Pool__Size ---------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_Pool__Size( TgUINT_F32_P NONULL pnbyReserve, TgUINT_F32_P NONULL pnbyAllocated, TgVOID_CPC pMem )
{
    union
    {
        TgVOID_CP                           pVoid;
    ...
    uMem.uiPTR = tgCM_CEL_ALGN_PW2_UMAX( (TgUINT_PTR)pMem, s_uiPageSize );
    if (uMem.uiPTR >= s_uiPageSize)
    {
        uMem.uiPTR -= s_uiPageSize;
    };
    TgCRITICAL( 0 == (uMem.uiPTR % s_uiPageSize) );
...
static TgRESULT tgMM_Pool__Malloc( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP ppReturn, TgRSIZE_C uiSize )
{
    ...
    if (nullptr == uMem.psMem_Page)
    {
        ...
        else
        {
        ...
        #if TgCOMPILE__MEM_MALLOC_CLEAR
            tgMM_Set_U08( uMem.pVoid, s_uiPageSize, (int)TgCOMPILE_MEM_MALLOC_PATTERN );
        #endif
            ...
            uMem.psMem_Page->m_uiBlock_Size = (TgUINT_E32)uiBlock_Size;
...
#if TgCOMPILE__MEM_MALLOC_CLEAR
    tgMM_Set_U08( uRet.m_pVoid, uMem.psMem_Page->m_uiBlock_Size, (int)TgCOMPILE_MEM_MALLOC_PATTERN );
#endif
...
static TgRESULT tgMM_Pool__Free( TgMEM_INFO_HEADER_P psInfo, TgVOID_PC pMem )
{
    ...
    uPage.uiPTR = tgCM_CEL_ALGN_PW2_UMAX( (TgUINT_PTR)pMem, s_uiPageSize ) - s_uiPageSize;

    if (uPage.psMem_Page->m_uiBlock_Size > (TgUINT_F32)s_uiMaxBlock)
    {
    ...
    else
    {
        ...
        uBlock_Index.m_iPTR = uBlock_Index.m_puiE08 - uPage.psMem_Page->m_auiData;
        TgERROR(0 == (uBlock_Index.m_uiRSIZE % uPage.psMem_Page->m_uiBlock_Size));
        uBlock_Index.m_uiRSIZE /= uPage.psMem_Page->m_uiBlock_Size;
        TgERROR(uBlock_Index.m_uiRSIZE < (TgRSIZE)uPage.psMem_Page->m_uiBlock_Last);
        uiBlock_Index = (TgUINT_E16)(uBlock_Index.m_uiRSIZE & KTgMAX_U16);
        TgERROR( (TgVOID_P)(uPage.psMem_Page->m_auiData + uBlock_Index.m_uiRSIZE*uPage.psMem_Page->m_uiBlock_Size) == pMem );

        uiBitPool = (TgRSIZE)tgPM_BSR_UMAX( uPage.psMem_Page->m_uiBlock_Size );
        uiPool_MSD = uiBitPool - KTgMIN_ALLOC_BIT;
        uiPool_LSD = (uPage.psMem_Page->m_uiBlock_Size >> (uiBitPool - 2)) & 3;
        uiIndex_Pool = uiPool_MSD * 4 + uiPool_LSD;

    #if TgCOMPILE__MEM_FREE_CLEAR
        tgMM_Set_U08( pMem, uPage.psMem_Page->m_uiBlock_Size, (int)TgCOMPILE_MEM_FREE_PATTERN );
    #endif
        ...
        while (!atomic_compare_exchange_strong( &uPage.psMem_Page->m_uiBlock_Free, &uiFree, uiBlock_Index ));
        tgCM_UT_LF__SN__Signal( &s_asPage_Spin_Lock[uiIndex_Pool].m_sLock );

        psInfo->m_uiSize = uPage.psMem_Page->m_uiBlock_Size;
        psInfo->m_uiFlags = uPage.psMem_Page->m_sInfo_Header.m_uiFlags;

        /* Check to see if this has re-opened a formerly closed page. */
        if (nullptr != uPage.psMem_Page->m_sList_Header.m_pPrev_Node)
        {
            tgCM_UT_LF__SN__Lock_Spin( &s_asPage_Spin_Lock[uiIndex_Pool].m_sLock );
            if (nullptr != uPage.psMem_Page->m_sList_Header.m_pPrev_Node)
            {
                tgCM_UT_ST__LT__Remove( &uPage.psMem_Page->m_sList_Header );
                uPage.psMem_Page->m_sList_Header.m_pNext_Node = nullptr;
                uPage.psMem_Page->m_sList_Header.m_pPrev_Node = nullptr;

                uPage.psMem_Page->m_sStack_Header.m_pNext_Node = nullptr;
                tgCM_UT_LF__ST__Push( &s_asPage_Open[uiIndex_Pool].m_sStack, &uPage.psMem_Page->m_sStack_Header );
            };
            tgCM_UT_LF__SN__Signal( &s_asPage_Spin_Lock[uiIndex_Pool].m_sLock );
        };
...
/* ---- tgMM_Pool__Realloc ------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_Pool__Realloc( TgMEM_INFO_HEADER_P NONULL psNew_Info, TgVOID_PP NONULL ppNew,
                                    TgMEM_INFO_HEADER_P NONULL psOld_Info, TgVOID_PC NONULL pOld, TgRSIZE_C uiNewSize )
{
    union
    {
        TgUINT_PTR                          uiPTR;
    ...
    uPage.uiPTR = tgCM_CEL_ALGN_PW2_UMAX( (TgUINT_PTR)pOld, s_uiPageSize ) - s_uiPageSize;
...
TgVOID tgMM_Pool__Stats( STg2_Output_PC psOUT )
{
    for (TgRSIZE uiIndex = 0; uiIndex < KTgMAX_POOLS; ++uiIndex)
    {
        ...
        tgIO_PrintF_NCZ( psOUT, "Pool(% 8d)               % 8d    % 8d\n", uiBlockSize, uiTotalSize, nuiAlloc );
    };
}
/*# TgS_STAT__COMMON */
#endif
...
static TgBOOL tgMM_Pool__Check_Page_Is_Empty( TgMEM_PAGE_HEADER_P psMem_Page )
{
    ...
    for (TgUINT_E16 uiCheck = 0; uiCheck < psMem_Page->m_uiBlock_Last; ++uiCheck)
    {
        ...
        TgLOG_MEM( "Mem_MGR", "%-16.16s(%-32.32s): Unfreed Memory Allocation at (% 16p).\n", "Common", "MM_Free_MGR", sUnion.m_pVoid );
    };
