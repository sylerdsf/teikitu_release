...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL FCN_VO(tgGM_DK_Is_Valid)( VEC_OBJ_T(TgDISK,CPCU) psDK0 )
{
    if (!FCN_V(tgMH_Is_Valid_Reference_Frame,3)( &psDK0->m_mReference_Frame ))
    {
        return (false);
    };

    if (!(psDK0->m_fRadius > TYPE_K(0)) || !(psDK0->m_fRadiusSq > TYPE_K(0)))
    {
        return (false);
    };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
VEC_T(1) FCN_VO(tgGM_DK_Support_Point)( VEC_OBJ_T(TgDISK,CPCU) psDK0, VEC_T(1,C) vDN )
{
    TgERROR(FCN_V(tgMH_Is_Valid_Unit_Vector)( vDN ));

    VEC_T(1,C)      vN0 = FCN_V(tgMH_Query_Axis_1,3)( &psDK0->m_mReference_Frame );
    VEC_T(1,C)      vDN_N0 = FCN_V(tgMH_DOT)( vDN, vN0 );
    VEC_T(1,C)      vC0 = FCN_V(tgMH_Query_Axis_3,3)( &psDK0->m_mReference_Frame );
    ...
    VEC_T(1,C)      vX3 = FCN_V(tgMH_MUL)( vX2, FCN_V(tgMH_SET1)( psDK0->m_fRadius ) );
...
/* ---- FCN_VO(tgGM_DK_BA) ------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID FCN_VO(tgGM_DK_BA)( VEC_OBJ_T(TgBOXAA,PCU) psBA0, VEC_OBJ_T(TgDISK,CPCU) psDK0 )
{
    TgGEOM_ASSERT_PARAM(FCN_VO(tgGM_DK_Is_Valid)( psDK0 ));

    VEC_UN_T(1,C)   uN0 = { VEC_UN_1_MEMBER() = FCN_V(tgMH_Query_Axis_1,3)( &psDK0->m_mReference_Frame ) };
    VEC_T(1,C)      vNEG_N0 = FCN_V(tgMH_NEG)( VEC_UN_1_MEMBER(uN0) );
    VEC_UN_T(1,C)   uC0 = { VEC_UN_1_MEMBER() = FCN_V(tgMH_Query_Axis_3,3)( &psDK0->m_mReference_Frame ) };
    ...
    if (!FCN_F(tgCM_NR1)( FCN_F(tgPM_ABS)( VEC_S_UN_1_MEMBER(uN0).x ) ))
    {
        ...
        VAR_T(C)        fRad_A = FCN_F(tgPM_ABS)( psDK0->m_fRadius * VEC_S_UN_1_MEMBER(uU1).x );
    ...
    if (!FCN_F(tgCM_NR1)( FCN_F(tgPM_ABS)( VEC_S_UN_1_MEMBER(uN0).y ) ))
    {
        ...
        VAR_T(C)        fRad_A = FCN_F(tgPM_ABS)( psDK0->m_fRadius * VEC_S_UN_1_MEMBER(uU1).y );
    ...
    if (!FCN_F(tgCM_NR1)( FCN_F(tgPM_ABS)( VEC_S_UN_1_MEMBER(uN0).z ) ))
    {
        ...
        VAR_T(C)        fRad_A = FCN_F(tgPM_ABS)( psDK0->m_fRadius * VEC_S_UN_1_MEMBER(uU1).z );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID FCN_VO(tgGM_DK_Project)( VEC_T(1,PCU) pvMin, VEC_T(1,PCU) pvMax, VEC_OBJ_T(TgDISK,CPCU) psDK0, VEC_T(1,C) vDN )
{
    TgGEOM_ASSERT_PARAM(nullptr != pvMin && nullptr != pvMax && FCN_VO(tgGM_DK_Is_Valid)( psDK0 ) && FCN_V(tgMH_Is_Valid_Unit_Vector)( vDN ));
    ...
    {
        *pvMin = FCN_V(tgMH_Init_Vector)( FCN_V(tgMH_SET1)( VAR_K(KTgMAX) ) );
        *pvMax = FCN_V(tgMH_Init_Vector)( FCN_V(tgMH_SET1)( -VAR_K(KTgMAX) ) );
    }
    else
    {
        ...
        VEC_T(1,C)      vX2_DN = FCN_V(tgMH_ABS)( FCN_V(tgMH_DOT)( vDN, vX2 ) );
        VEC_T(1,C)      vRad_AX = FCN_V(tgMH_MUL)( FCN_V(tgMH_SET1)( psDK0->m_fRadius ), vX2_DN );
