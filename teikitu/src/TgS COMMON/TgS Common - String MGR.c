...
TgTYPE_STRUCT(STg2_SM_String_Entry,)
{
    ...
    TgSTRING_DICT_ID_SINGLETON                  m_tiString_Singleton;
};
...
static TgCHAR_MB_CP                         s_apszNative_String[KTgSM_MAX_NATIVE_ENTRIES];
...
static TgSTRING_TABLE_ID_SINGLETON          s_atiString_Table_Singleton[KTgSM_NUM_STRING_TABLE_MAX];
...
TgRESULT tgSM_Init_MGR( TgVOID )
{
    ...
    {
        atomic_store( s_auiGlobal_Dict_Hash + uiIndex, -1 );
    };
    ...
    {
        tgSTRING_TABLE_ID_Store( s_atiString_Table_Singleton + uiIndex, KTgSTRING_TABLE_ID__INVALID );
    };
...
TgRESULT tgSM_Boot_MGR( TgVOID )
{
    ...
    #define NATIVE_LITERAL( szLiteral ) tgSM_UID_Init_String( #szLiteral, __LINE__ - NATIVE_LITERAL_START_LINE );
...
TgRESULT tgSM_Stop_MGR( TgVOID )
{
    ...
    {
        if (!tgSTRING_TABLE_ID_Fetch_And_Is_Valid( &tiString_Table, s_atiString_Table_Singleton + iIndex))
        {
            continue;
        };
        TgWARNING_MSGF( 0, "%-16.16s(%-32.32s): String Table (%d) was not removed.\n", "String MGR", "tgSM_Stop_MGR", iIndex );
...
TgRSIZE tgSM_Query_Fixed_Memory( TgVOID )
{
    return (0
             ...
             + sizeof( s_atiString_Table_Singleton )
...
TgVOID tgSM_Stats( STg2_Output_P psOUT )
{
    ...
    {
        TgCHAR_MB_CP                        mbzNativeString;

        mbzNativeString = s_apszNative_String[iIndex];
        if (nullptr == mbzNativeString)
            continue;

        tgIO_PrintF_NCZ( psOUT, "%-16.16s: Native String (%d, %s).\n", "String MGR", iIndex, mbzNativeString );
    };
    ...
    for (uiOffset = 0; uiOffset < uiMax_Offset;)
    {
        ...
        tgIO_PrintF_NCZ( psOUT, "%-16.16s: Global String (%d, %s).\n", "String MGR", iIndex, (TgCHAR_MB_P)(psEntry.ps + 1) );
    ...
    for (iIndex = 0; iIndex < KTgSM_NUM_STRING_TABLE_MAX; ++iIndex)
    {
        ...
        if (!tgSTRING_TABLE_ID_Fetch_And_Is_Valid( nullptr, s_atiString_Table_Singleton + iIndex ))
        {
            continue;
        };

        uiEntryMax = s_apsString_Table[iIndex]->m_nuiEntry_ID;
        tgIO_PrintF_NCZ( psOUT, "%-16.16s: Table (%d) with (%d) entries.\n", "String MGR", iIndex, uiEntryMax );
        ...
        for (TgRSIZE uiEntry = 0; uiEntry < uiEntryMax; ++uiEntry)
        {
            ...
            tgIO_PrintF_NCZ( psOUT, "%-16.16s:  - String (%d, %s)\n", "String MGR", uiEntry, sData.psEntry->m_mbzEntry );
        };
    };
}
/*# TgS_STAT__COMMON */
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSINT_F32 tgSM_UID_Init_String( TgCHAR_MB_CPC mbzNative_String, TgSINT_F32_C iIndex )
{
    TgDIAG( ETgMODULE_STATE__BOOTED == s_enString_MGR_State );

    if (nullptr == mbzNative_String || (nullptr == mbzNative_String[0]))
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Invalid String.\n", "String MGR", "tgSM_UID_Init_String" );
        return (-1);
    }

    if (iIndex < 0 || iIndex >= KTgSM_MAX_NATIVE_ENTRIES || 0 != s_apszNative_String[iIndex])
    {
        TgCRITICAL_MSGF( 0, "%-16.16s(%-32.32s): Invalid Index.\n", "String MGR", "tgSM_UID_Init_String" );
        return (-1);
    }
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgCHAR_MB_CP tgSM_UID_Query_String( TgSINT_F32_C iIndex )
{
    TgDIAG( ETgMODULE_STATE__BOOTED == s_enString_MGR_State );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSTRING_DICT_ID tgSM_Dict_Insert_String_Hash( TgCHAR_MB_CPC NONULL mbzString, TgRSIZE_C nbyMaxString, TgUINT_MAX_C uiHash )
{
    ...
    TgDIAG( ETgMODULE_STATE__BOOTED == s_enString_MGR_State );
    TgPARAM_CHECK(nullptr != mbzString && nbyMaxString > 0 && ((nbyMaxString <= KTgMAX_RSIZE) || (KTgMAX_SIZE_ALL == nbyMaxString)));
    ...
    nbyString_Encode_Size = tgMBZ_Length_U08( mbzString, nbyMaxString ) + 1;
    ...
    tiRet = tgSTRING_DICT_ID_Init( &psEntry.ps->m_tiString_Singleton, (TgUINT_F32)uiOffset );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSTRING_DICT_ID tgSM_Dict_Insert_String( TgCHAR_MB_CPC mbzString, TgRSIZE_C nbyMaxString )
{
    TgUINT_MAX                          uiHash;

    if (TgFAILED(tgMBZ_Hash( &uiHash, mbzString, nbyMaxString )))
    {
        return (KTgSTRING_DICT_ID__INVALID);
    };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSTRING_DICT_ID tgSM_Dict_Find_Id_By_Hash( TgCHAR_MB_CPC mbzString, TgUINT_MAX_C uiHash )
{
    TgSINT_F32                          iHash_Index_Start;
    ...
    do
    {
        uiOffset = atomic_load( s_auiGlobal_Dict_Hash + iHash_Index );
        ...
        psEntry.pui = s_pGlobal_Dict_Data + uiOffset;
        if (TgSUCCEEDED(tgMBZ_Compare( &iRet, (TgCHAR_MB_P)(psEntry.ps + 1), KTgMAX_SIZE_ALL, 0, mbzString, KTgMAX_SIZE_ALL, 0 )) && (0 == iRet))
        {
            return (tgSTRING_DICT_ID_Query_Unsafe(&psEntry.ps->m_tiString_Singleton));
        };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSTRING_DICT_ID tgSM_Dict_Find_Id_By_String( TgCHAR_MB_CPC mbzString, TgRSIZE_C nbyMaxString )
{
    TgUINT_MAX                          uiHash;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgSM_Dict_Query_String( TgCHAR_MB_CPP mbzStringOut, TgRSIZE_PC piLengthOut, TgSTRING_DICT_ID_C tiString )
{
    union
    {
        STg2_SM_String_Entry_P              ps;
    ...
    if (!tgSTRING_DICT_ID_Is_Equal( &psEntry.ps->m_tiString_Singleton, tiString ))
    {
        TgERROR_MSGF( 0, "%-16.16s(%-32.32s): Stale String Dictionary ID.\n", "Common", "tgSM_Dict_Decrement_Reference" );
        return (false);
    }
    ...
    if (mbzStringOut)
        *mbzStringOut = (TgCHAR_MB_P)(psEntry.ps + 1);
    if (piLengthOut)
        *piLengthOut = psEntry.ps->m_sEntry_Id.m_uiSize - 1;
...
TgSTRING_TABLE_ID tgSM_Table_Load( STg2_Input_PC psInput, TgRSIZE_C uiOffset_Start )
{
    ...
    for (TgUINT_F32 uiIndex = 1; uiIndex < KTgSM_NUM_STRING_TABLE_MAX; ++uiIndex)
    {
        ...
        if (!atomic_compare_exchange_strong( &s_atiString_Table_Singleton[uiIndex].m_uiKI, &tiRet.m_uiKI, KTgMAX_U32))
        {
        ...
        tiRet = tgSTRING_TABLE_ID_Init( s_atiString_Table_Singleton + uiIndex, uiIndex );

        s_apsString_Table[uiIndex] = TgMALLOC_POOL( sTable.m_nbyData );
...
TgBOOL tgSM_Table_Free( TgSTRING_TABLE_ID_C tiTable )
{
    ...
    tgSTRING_TABLE_ID_Invalidate(s_atiString_Table_Singleton + tiTable.m_uiI );
...
{
    return (tgSTRING_TABLE_ID_Is_Equal( s_atiString_Table_Singleton + tiTable.m_uiI, tiTable ));
}


/* ---- tgSM_Table_Query_String -------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgSM_Table_Query_String( TgCHAR_MB_CPP NONULL mbzStringOut, TgRSIZE_PC NONULL piLengthOut, TgSTRING_TABLE_ID tiTable, TgUINT_F32_C uiIndex )
{
    union
    {
        STg2_String_Table_Entry_CP          psEntry;
    ...
    *piLengthOut = sEntry_Id.m_uiSize - 1;
