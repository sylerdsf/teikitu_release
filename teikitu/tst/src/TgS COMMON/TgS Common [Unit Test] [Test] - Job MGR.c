...
TEST_METHOD( UNIT_TEST__TEST__JM_Init_Job_Queue )
{
    ...
    tiJob_Thread = tgJM_Spawn_Job_Thread( atiQueueList, 4, ETgTHREAD_PRIORITY__NORMAL, "Job_Unit_Test" );
...
TEST_METHOD( UNIT_TEST__TEST__JM_Spawn_Job_Thread )
{
    STg2_Job                            sJob_Test;
    TgFLOAT32                           fTime;
    TgCHAR_MB                           szBuffer[2048];
    TgCHAR_MB                           szExecuteTime[256];
    TgBOOL                              bMT_Enabled;
    ...
    {
        atiJob_Thread[uiJobThread] = tgJM_Spawn_Job_Thread( atiQueueList, 4, ETgTHREAD_PRIORITY__NORMAL, "Job_Unit_Test" );
    ...
    atiJob_Thread[KTgMAX_NUM_THREAD] = tgJM_Spawn_Job_Thread( atiQueueList, 4, ETgTHREAD_PRIORITY__NORMAL, "Job_Unit_Test" );
...
/*# TgCOMPILE__THREAD */
    ...
    tgMBZ_PrintF( szExecuteTime, TgARRAY_COUNT( szExecuteTime ), "% 8.3f ms", (TgFLOAT64)(tgTM_Query_Time() - fTime) );
    bMT_Enabled = false;
    bMT_Enabled &= tgCN_Var_Query_Bool( &bMT_Enabled, g_tiCVAR_Job_MGR_Enabled );
    if (bMT_Enabled)
    {
        tgMBZ_PrintF( szBuffer, TgARRAY_COUNT( szBuffer ), UNIT_TEST__MSG_LITERAL_3, "INFO:", "Common: Common", "Common", "Job MGR (MT)", szExecuteTime );
    }
    else
    {
        tgMBZ_PrintF( szBuffer, TgARRAY_COUNT( szBuffer ), UNIT_TEST__MSG_LITERAL_3, "INFO:", "Common: Common", "Common", "Job MGR (ST)", szExecuteTime );
    };
    TgMSGF( 0, "%s", szBuffer );
